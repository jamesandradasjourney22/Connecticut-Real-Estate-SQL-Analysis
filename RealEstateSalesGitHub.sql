...
-- By exploring serial numbers, it's clear that properties situated close to each other tend to have more similar serial number patterns. 
-- This suggests a link between how near properties are to each other and how their serial numbers appear, offering an interesting aspect to the dataset's insights.
SELECT * FROM realestatesales
ORDER BY serial_number DESC;


-- 2. The following SQL query checks whether there are duplicate entries within the dataset. 
-- There are 0 duplicate entries within the dataset. 
SELECT 
...
-- LIMITATION OF DATASET: The dataset comprises 388,304 rows where the residential type is indicated as an empty space (''). 
-- Furthermore, there are an additional 4 rows in the dataset where the residential type is marked as NULL.
SELECT DISTINCT
...
GROUP BY sale_range
ORDER BY sale_range;
-- After analyzing the result set, it's evident that the distribution of sale amounts by sales range follows an anticipated right-skewed pattern. 
-- This means that as the property sale amount increased, the number of properties sold decreased, aligning with typical real estate market trends.
...
-- POTENTIAL DATA ENTRY ERROR/ DATA DISCREPANCY: A total of 56 instances were identified in the dataset where the recorded sale date precedes the property's listing date, indicating a noteworthy data anomaly where sale transactions appear to have been recorded prior to the property being officially listed. 
-- Furthermore, within the dataset, two rows feature NULL values in the recorded date field. Given this circumstance, and in preparation for calculating three-year moving averages to glean insights into yearly sales trends, I opted to exclude these rows from my calculation above.
...
-- Cohort Analysis Definition: A type of behavioral analysis that breaks data into related groups before analysis. 
...
-- DATA ENTRY ERROR: In the dataset, there's one record without a specified town. 
-- However, with a quick inquiry (using google search), it's clear that the correct town is East Hampton, Connecticut. 
-- The following queries highlight the specific record labeled as "***unknown***" for the town and correct it by changing it to the accurate town, East Hampton.
SELECT *
FROM realestatesales
where town = '***Unknown***';
